...
PlayerAI::PlayerAI(Game* game, Player* player)
    : QObject(game)
    , m_game(game)
    , m_player(player)
    , m_arena(nullptr)
...
std::vector<std::vector<bool>> PlayerAI::computeDangerGrid(const QPoint& simulatedBombPos)
{
    std::vector<std::vector<bool>> danger(m_rows, std::vector<bool>(m_cols, false));
    QList<Bomb*> bombs = m_game->getBombs();
    ...
    if (simulatedBombPos.x() != -1 && simulatedBombPos.y() != -1) {
        ...
        if (r >= 0 && r < m_rows && c >= 0 && c < m_cols) {
            danger[r][c] = true;
            int power = m_player->getBombPower();
            int rowOffsets[] = {-1, 1, 0, 0};
            int colOffsets[] = {0, 0, -1, 1};
            for (int i = 0; i < 4; ++i) {
                for (int step = 1; step <= power; ++step) {
...
bool PlayerAI::isGreenBonus(int row, int col)
{
...
bool PlayerAI::isBadBonus(int row, int col)
{
    QList<Element*> elements = m_arena->getCell(row, col).getElements(Granatier::Element::BONUS);
    for (auto* element : elements) {
        auto* bonus = dynamic_cast<Bonus*>(element);
...
bool PlayerAI::isAdjacentToBlock(int row, int col)
{
    int rowOffsets[] = {-1, 1, 0, 0};
    int colOffsets[] = {0, 0, -1, 1};
    for (int i = 0; i < 4; ++i) {
...
bool PlayerAI::isAdjacentToEnemy(int row, int col)
{
    QList<Player*> players = m_game->getPlayers();
    int power = m_player->getBombPower();
    for (auto* other : players) {
        if (other == m_player || !other->isAlive()) continue;
        
        // Teammate check (team 0 means Solo, teams 1+ are shared teammate groups)
        if (m_player->team() != 0 && m_player->team() == other->team()) continue;
...
QList<QPoint> PlayerAI::findPath(const QPoint& start, const std::vector<std::vector<bool>>& dangerGrid, int targetType)
{
...
QList<QPoint> PlayerAI::findPathImpl(const QPoint& start, const std::vector<std::vector<bool>>& dangerGrid, int targetType, bool strictlySafe)
{
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (nr >= 0 && nr < m_rows && nc >= 0 && nc < m_cols && !visited[nr][nc]) {
                ...
                bool walkable = (isStartCell || m_arena->getCell(nr, nc).isWalkable(m_player)) &&
...
void PlayerAI::moveTowardsCell(const QPoint& targetCell)
{
    int myRow = m_arena->getRowFromY(m_player->getY());
    int myCol = m_arena->getColFromX(m_player->getX());
    ...
    qreal currX = m_player->getX();
    qreal currY = m_player->getY();
    ...
    if (targetCell.x() != myCol && targetCell.y() != myRow) {
        ...
        if (std::abs(dx) > tolerance) {
            if (dx > 0) m_player->goRight();
            else m_player->goLeft();
        } else if (std::abs(dy) > tolerance) {
            if (dy > 0) m_player->goDown();
            else m_player->goUp();
        } else {
            m_player->stopMoving();
        }
...
{
    if (!m_player->isAlive() || m_player->isFalling()) {
        m_currentTargetCell = QPoint(-1, -1);
        m_currentPath.clear();
        return;
    }

    Arena* currentArena = m_game->getArena();
    ...
    if (m_currentTargetCell.x() != -1 && m_currentTargetCell.y() != -1) {
        ...
        if (m_currentTargetCell.x() < 0 || m_currentTargetCell.x() >= m_cols ||
            ...
            bool isWalkable = m_arena->getCell(m_currentTargetCell.y(), m_currentTargetCell.x()).isWalkable(m_player) &&
    ...
    if (adjacentToTarget && m_player->bombArmory() > 0 && dangerGrid[myRow][myCol] == false) {
        ...
        QList<Bomb*> bombs = m_game->getBombs();
        for (auto* bomb : bombs) {
            if (!bomb->isDetonated() && bomb->creator() == m_player) {
...
QList<QPoint> PlayerAI::findPathToEnemyDirectly(const QPoint& start, const std::vector<std::vector<bool>>& dangerGrid)
{
    QList<Player*> players = m_game->getPlayers();
